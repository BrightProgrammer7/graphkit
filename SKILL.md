...
8. **Hosts** — where will each node run? Executor: a Grok `/goal` session (continuous, low-babysit — the default when the user is on Grok), a shell/cron loop, or manual paste. Supervisor: Claude `CronCreate`, another scheduler, or a fresh session each interval. Mixed (cheap executor + strong supervisor) is first-class. This only picks which adapter snippets land in `LAUNCH.md`; it never changes the graph.
...
Also write a short **`LAUNCH.md`** into the run directory: the copy-paste launch snippet for each chosen host, filled from `templates/hosts/` with the run's real `{{RUN_DIR}}` path — executor snippet (`grok-goal.md` thin launcher / `manual-paste.md`) and, if a supervisor was chosen, its schedule snippet (`claude-cron.md`). `LAUNCH.md` is a convenience index, not a scoreboard; it points at the files, it doesn't copy their contents.

**Step 3 — Start the executor** in a **fresh context** on the chosen host. For a Grok `/goal` host, use the **thin launcher** from `templates/hosts/grok-goal.md` — a one-line pointer at the run files, **not** the whole `executor.md` pasted in — so the goal runs continuously across rounds and never drifts from the file. (Pasting the full prompt still works as a manual fallback.) The host only keeps the node alive across turns; the executor's authority is `executor.md` + the ledger. Keep the ledger and directives as **live files the node re-reads each round** — never fold their content into the host's own goal/prompt text, where it goes stale. A host's progress UI (a goal's done-bar) mirrors the ledger, never replaces it; the ledger wins every conflict. It's self-contained (points at ledger + ops) and runs fine on a **cheap/fast model** — structure, not model, keeps it on-spec.
...
- `templates/hosts/` — copy-paste host adapters that launch a node: `grok-goal.md` (executor via `/goal`, continuous), `claude-cron.md` (supervisor via cron), `manual-paste.md` (fallback). Hosts keep a node alive; they never become a second scoreboard.
